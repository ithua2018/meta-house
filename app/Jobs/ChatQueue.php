...
class ChatQueue implements ShouldQueue
{
    use InteractsWithQueue, Queueable, SerializesModels;

    private $data;
    private $socketClientService;
    private $server_run_id = '';
    /**
     * 消息事件与回调事件绑定
     *
     * @var array
...
//            }
     ...
     * @return string
     */
    public function onConsumeOnlineStatus(array $data,  $message): bool
    {
...
//        $this->socketPushNotify(array_unique($fds), json_encode([SocketConstants::EVENT_ONLINE_STATUS, $data['data']]));
     ...
     * @return string
     */
    public function onConsumeTalk(array $data,   $message): bool
    {

        $fds = array_merge(
            $this->socketClientService->findUserFds($data['data']['sender'], $this->server_run_id),
            $this->socketClientService->findUserFds($data['data']['receive'], $this->server_run_id)
        );
